...
// Team's existing dedup setting (24 hours)
...
export const sendNotificationToAdmins = async (
  ...
  for (const adminId of admins) {
    ...
    try {
      const notif = await sendNotification(adminId, text, type, meta, options);
      created.push(notif);
    } catch (err) {
...
//  Delete notifications for a specific user when follow-up changes
export const deleteNotificationsByEntity = async (entityType, entityId, userId = null) => {
  try {
    ...
    if (result.deletedCount > 0) {
      ...
      if (userId) {
        const ids = notifications.map((notif) => String(notif._id));
...
//  Delete ALL notifications for an entity (for cascade delete - admins + users)
export const deleteAllNotificationsByEntity = async (entityType, entityId) => {
  try {
    const query = buildEntityNotificationQuery(entityType, entityId);
    ...
    if (result.deletedCount > 0) {
      ...
      notifications.forEach((notif) => {
        ...
        if (!deletionsByUser.has(userString)) {
          deletionsByUser.set(userString, []);
        }
        deletionsByUser.get(userString).push(String(notif._id));
      });
      for (const [userId, ids] of deletionsByUser.entries()) {
        notifyUser(userId, "notification_deleted", { ids });
      }
    }
