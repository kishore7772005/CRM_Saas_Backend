...
const callLogSchema = new mongoose.Schema(
  {
    ...
    callType:   { type: String, enum: ["whatsapp", "phone"], default: "phone" },
    ...
    userAgent:   String,
    ipAddress:   String,
    initiatedBy: { type: String, enum: ["user", "bot"], default: "user" },
...
callLogSchema.pre("save", function (next) {
  if (this.startTime && this.endTime) {
    ...
    if (this.duration > 0) this.callStatus = "completed";
    else if (this.duration === 0 && this.startTime) this.callStatus = "missed";
  }
  next();
});
...
callLogSchema.index({ dealId: 1, createdAt: -1 });
