...
const roleController = {
  ...
  getRoles: async (req, res) => {
    try {
      ...
      const [roles, total] = await Promise.all([Role.find().skip(skip).limit(limit), Role.countDocuments()]);
      res.status(200).json({ roles, total, page, pages: Math.ceil(total / limit) });
    } catch (err) {
