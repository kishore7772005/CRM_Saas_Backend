...
export default {
  ...
  convertLeadToDeal: async (req, res) => {
    try {
      ...
      const deal = new Deal({
        leadId: lead._id, dealName: lead.dealName, assignedTo: lead.assignTo?._id ?? null,
        value: formattedValue, currency: currency || "INR", notes: notes || "",
        stage: stage || "Qualification", email: lead.email || "", phoneNumber: lead.phoneNumber || "",
        source: lead.source || "", companyName: lead.companyName || "", industry: lead.industry || "",
        requirement: lead.requirement || "", country: lead.country || "", address: lead.address || "",
        clientType: lead.clientType || "", attachments: lead.attachments || [],
        followUpDate: lead.followUpDate ?? null, lastReminderAt: lead.lastReminderAt ?? null,
      });
      await deal.save();
