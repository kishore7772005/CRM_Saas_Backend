...
export default {
  createDealFromLead: async (req, res) => {
    try {
      ...
      const deal = new Deal({ leadId: lead._id, dealName: lead.leadName, assignedTo: lead.assignTo?._id, stage: "Qualification", value: "0" });
      await deal.save();
  ...
  createManualDeal: async (req, res) => {
    try {
      ...
      const deal = new Deal({
        dealName, assignedTo: assignTo || null, value: formattedValue, currency: currency || "INR",
        stage: dealStage, notes: notes || "", phoneNumber, email: email || "", source: source || "",
        companyName: companyName || "", companyId: companyId || null, industry: industry || "",
        requirement: requirement || "", address: address || "", country: country || "",
        clientType: clientType || "", followUpDate: parsedFollowUpDate, followUpComment: followUpComment || "",
        followUpHistory, lossReason: lossReason || "", lossNotes: lossNotes || "", attachments,
      });
      await deal.save();
